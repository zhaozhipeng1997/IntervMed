...
def Mydebias(model,LAYER_IDX):
    ...
    if "l" in LAYER_IDX:
        ...
        model.vision_model.encoder.layers[IDX].mlp.fc1.weight.data *= 0.7
        model.vision_model.encoder.layers[IDX].mlp.fc2.weight.data *= 0.7
    elif "q" in LAYER_IDX:
        model.qformer.encoder.layer[IDX].intermediate.dense.weight.data *= 0.7
        model.qformer.encoder.layer[IDX].output.dense.weight.data *= 0.7
        if hasattr(model.qformer.encoder.layer[IDX], 'intermediate_query'):
            model.qformer.encoder.layer[IDX].intermediate_query.dense.weight.data *= 0.7
            model.qformer.encoder.layer[IDX].output_query.dense.weight.data *= 0.7
...
def analyze_layer_impact(processor, model, device, bias_img, bias_txt, unbias_img, unbias_txt,
    ...
    bias_inputs = processor(images=bias_img, text=bias_txt, return_tensors="pt").to(device)
    
    unbias_inputs = processor(images=unbias_img, text=unbias_txt, return_tensors="pt").to(device)
...
def opera_analyze_layer_impact(processor, model, device, bias_img, bias_txt, unbias_img, unbias_txt,
                         target_layer="multi_modal_projector", max_new_tokens=20, intervention_strength=0.8):
    return analyze_layer_impact(processor, model, device, bias_img, bias_txt, unbias_img, unbias_txt,
                                target_layer=target_layer, max_new_tokens=max_new_tokens, intervention_strength=intervention_strength)
...
def get_layer(model, layer_spec):
    try:
        if layer_spec == "multi_modal_projector":
            ...
            return model.vision_model.encoder.layers[layer_index]
        elif layer_spec.startswith("l:"):
            layer_index = int(layer_spec.split(":")[1])
            return model.language_model.model.layers[layer_index]
        elif layer_spec.startswith("q:"):
            layer_index = int(layer_spec.split(":")[1])
            return model.qformer.encoder.layer[layer_index]
        else:
